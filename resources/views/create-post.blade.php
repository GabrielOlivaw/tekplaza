...
@section('content')
<div id="create-post-view" class="forum-item-list">
	<h1>@lang('website.thread-respond')</h1>
	...
	<form id="create-post-form" action="{{ route('create-post', ['subforum' => $subforum, 'thread' => $thread]) }}" method="post">
		@csrf
		<div id="register-form-title" class="register-form-field">
			<label for="title">@lang('website.thread-title')</label>
			<input type="text" id="title" class="form-control" name="title" value="RE: {{ $thread->title }}" required>
		</div>
